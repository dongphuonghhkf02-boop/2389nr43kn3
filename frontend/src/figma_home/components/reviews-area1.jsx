  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const data = r?.data || {};
        // Pull Avito social url for the public "reviews on Avito" CTA
        const socials = data?.socials || {};
        const avito = socials?.avito;
        const avitoLink = (avito && (avito.url || avito.link)) || "";
        if (avitoLink) setAvitoUrl(avitoLink);
    ...
    >
      <div className={styles.layout}>
        ...
        <aside className={styles.infoColumn}>
          ...
          {/* Verified Reviews card — replaces previous Google branding per
              product policy: we no longer redirect public visitors to
              Google Maps and do not expose the Google logo on the public
              surface. Rating + count are still surfaced (sourced from
              moderation queue / Google sync), but neutrally labelled.
              When the configured Avito profile URL is present we surface
              a secondary CTA pointing at Avito reviews — our public
              track record. */}
          <div className={styles.googleCard} data-testid="reviews-card">
            ...
            <div className={styles.googleBlock}>
              ...
              <div className={styles.googleMeta}>
                ...
                <span className={styles.googleReviewsLink}>
                  {cfg.google_reviews_count} {lang === "ru" ? "проверенных отзывов" : "verified reviews"}
                </span>
                {avitoUrl ? (
                  <a
                    href={avitoUrl}
                  ...
                  >
                    {lang === "ru" ? "Отзывы на Avito" : "Reviews on Avito"}
                    <span aria-hidden="true">→</span>
                  </a>
                ) : null}
              </div>
