    ...
    <div data-testid="admin-info-page">
      ...
      */}
      ...
      <div className="min-w-0 space-y-5">
          ...
          {/* About Us page editor — hero, intro, gallery, stats, values, CTA */}
          {tab === 'about' && (
            <AboutEditor
              data={data}
              setData={setData}
            ...
            <div className="space-y-5">
              <Block title={t('navContacts')} description={t('adm_phones_email_addresses_and_working_hours_displayed')}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-4">
                    <Field label={t('emailLabel')}>
                      <input
                        ...
                        className={inputCls}
                        placeholder="info@dm-auto.online"
                        data-testid="info-footer-email"
                      />
                    </Field>
                  </div>

                  {/* WhatsApp / Telegram direct-message handles — used by the
                      Contacts page channel cards and the public Get-In-Touch
                      modal. Phone is normalized to E.164 (no spaces) so wa.me
                      and t.me links work without any client-side parsing. */}
                  <Field label="WhatsApp number (E.164)" hint="Used to build wa.me/&lt;number&gt; deep-link. Example: +375291234567">
                    <input
                      type="text"
                      value={data?.footer?.contacts?.whatsapp_number || ''}
                      onChange={(e) => updateFooter('contacts.whatsapp_number', e.target.value)}
                      className={inputCls}
                      placeholder="+375291234567"
                      data-testid="info-footer-whatsapp-number"
                    />
                  </Field>
                  <Field label="WhatsApp label (display)" hint="Visible to visitors. Example: +375 29 123 45 67">
                    <input
                      type="text"
                      value={data?.footer?.contacts?.whatsapp_label || ''}
                      onChange={(e) => updateFooter('contacts.whatsapp_label', e.target.value)}
                      className={inputCls}
                      placeholder="+375 29 123 45 67"
                      data-testid="info-footer-whatsapp-label"
                    />
                  </Field>
                  <Field label="Telegram username" hint="Without the @. Example: dm_auto_support → opens t.me/dm_auto_support">
                    <input
                      type="text"
                      value={data?.footer?.contacts?.telegram_username || ''}
                      onChange={(e) => updateFooter('contacts.telegram_username', e.target.value.replace(/^@/, ''))}
                      className={inputCls}
                      placeholder="dm_auto_support"
                      data-testid="info-footer-telegram-username"
                    />
                  </Field>

                  {/* Working hours — paired EN / RU */}
                  ...
                  <Field label="Часы работы (RU)" hint="Отображается посетителям, когда язык сайта — русский.">
                    <input
                      type="text"
                      value={data?.footer?.contacts?.working_hours_ru || ''}
                      onChange={(e) => updateFooter('contacts.working_hours_ru', e.target.value)}
                      className={inputCls}
                      placeholder="Пн – Вс · 24/7"
                      data-testid="info-footer-hours-ru"
                    />
                  </Field>

                  {/* Addresses — paired EN / RU */}
                  <Field label="Addresses (English)" hint={t('adm_one_address_per_line')}>
                    <textarea
                      ...
                      className={textareaCls}
                      placeholder={'Belarus, Minsk — by appointment\nRussia, Moscow — by appointment'}
                      data-testid="info-footer-addresses"
                    />
                  </Field>
                  <Field label="Адреса (RU)" hint="По одному адресу в строке. Подменяет английский вариант при переключении на русский.">
                    <textarea
                      rows={3}
                      value={(data?.footer?.contacts?.addresses_ru || []).join('\n')}
                      onChange={(e) =>
                        updateFooter(
                          'contacts.addresses_ru',
                          e.target.value.split('\n').map((s) => s.trim()).filter(Boolean),
                        )
                      }
                      className={textareaCls}
                      placeholder={'Беларусь, Минск — по записи\nРоссия, Москва — по записи'}
                      data-testid="info-footer-addresses-ru"
                    />
                  </Field>

                  {/* Registration address — paired EN / RU */}
                  <Field label="Registration address (English)">
                    <input
                      ...
                      className={inputCls}
                      placeholder="Belarus / Russia — by appointment"
                      data-testid="info-footer-reg-address"
                    />
                  </Field>
                  <Field label="Адрес регистрации (RU)">
                    <input
                      type="text"
                      value={data?.footer?.contacts?.registration_address_ru || ''}
                      onChange={(e) => updateFooter('contacts.registration_address_ru', e.target.value)}
                      className={inputCls}
                      placeholder="Беларусь / Россия — по записи"
                      data-testid="info-footer-reg-address-ru"
                    />
                  </Field>
                </div>
              </Block>

              {/* Lead-notification email — fires a Resend message to this address
                  every time a new public lead lands. Empty value disables the
                  email step (in-app notifications still happen). */}
              <Block
                title="Lead notifications · Email"
                description="When a visitor submits a form on the public site, we push a Resend email here in addition to the in-app notification. Leave blank to disable email notifications."
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Field label="Admin notification email" hint="Where new leads land (any inbox).">
                    <input
                      type="email"
                      value={data?.notifications?.admin_email || ''}
                      onChange={(e) =>
                        setData((prev) => ({
                          ...prev,
                          notifications: {
                      ...
                      className={inputCls}
                      placeholder="leads@dm-auto.online"
                      data-testid="info-notif-admin-email"
                    />
                  </Field>
                  <Field label="Subject prefix" hint="Prepended to every notification email.">
                    <input
                      type="text"
                      value={data?.notifications?.admin_email_subject_prefix || ''}
                      onChange={(e) =>
                        setData((prev) => ({
                          ...prev,
                          notifications: {
                            ...(prev?.notifications || {}),
                            admin_email_subject_prefix: e.target.value,
                          },
                        }))
                      }
                      className={inputCls}
                      placeholder="[DM Auto] "
                      data-testid="info-notif-admin-email-prefix"
                    />
                  </Field>
                  <Field label="Enabled" hint="Master switch — disables the email pipeline without losing the recipient.">
                    <label className="flex items-center gap-2 cursor-pointer select-none mt-2">
                      <input
                        type="checkbox"
                        checked={data?.notifications?.send_on_new_lead !== false}
                        onChange={(e) =>
                          setData((prev) => ({
                            ...prev,
                            notifications: {
                              ...(prev?.notifications || {}),
                              send_on_new_lead: e.target.checked,
                            },
                          }))
                        }
                        data-testid="info-notif-admin-email-enabled"
                      />
                      <span className="text-sm">Send Resend email on new lead</span>
                    </label>
                  </Field>
                </div>
              </Block>
    ...
    <div className="space-y-5">
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Field label="Section title (RU)">
            <input
              type="text"
              value={data?.faq?.title_ru || ''}
              onChange={(e) => updateFaq('title_ru', e.target.value)}
              className={inputCls}
              placeholder={t('adm_frequently_asked_questions')}
              data-testid="info-faq-title-ru"
            />
          </Field>
        </div>
      </Block>
              ...
              >
                <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-[#F4F4F5] bg-[#FAFAFA]">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-[13px] font-semibold text-[#18181B] truncate">
                      {item.question_en || item.question_ru || item.question_bg || <em className="text-[#A1A1AA] font-normal">{t('untitledQuestion')}</em>}
                    </span>
                  </div>
                ...
                <div className="p-4 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Field label="Question (RU)">
                      <input
                        type="text"
                        value={item.question_ru || ''}
                        onChange={(e) => updateFaqItem(idx, { question_ru: e.target.value })}
                        className={inputCls}
                        data-testid={`info-faq-q-ru-${idx}`}
                      />
                    </Field>
                  </div>
                  ...
                  <div>
                    <span className="block text-[12px] font-semibold text-[#52525B] mb-1.5 uppercase tracking-wider">Answer (RU)</span>
                    <div className="bibi-admin-quill">
                      <ReactQuill
                        theme="snow"
                        value={item.answer_ru || ''}
                        onChange={(v) => updateFaqItem(idx, { answer_ru: v })}
                        modules={quillModules}
                      />
                    </div>
                  </div>
