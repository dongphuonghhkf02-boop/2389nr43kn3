...
import {
  ...
  MobileQuickCalc,
  MobileThreeWays,
  MobileTurnkeyProcess,
  MobileAvitoProof,
  MobileReviews,
  MobileOurServices,
} from './MobileWelcomeBlocks';
...
const I18N = {
  en: {
    ...
    googleReviews: 'verified reviews',
  ...
  ru: {
    ...
    googleReviews: 'проверенных отзывов',
  ...
  return (
    ...
    >
      ...
      {/* OLD BLOCKS REMOVED (Phase 4 — mobile cleanup per user feedback):
         • MobileSearchFromAmericaKorea (popular brands)
         • MobileTopVehicleDeals (weekly deals w/ motorcycle filter)
         • MobileCalculateCar (old calc card)
         • MobileHowWeWork (old)
         • MobileHowToBuyTurnkey (old)
         • MobileTurnkeyServices (8 yellow cards — old design)
         • MobileBeforeAndAfter (old carousel)
         • MobileOurClientsSay (old Google-branded reviews)
         Kept: MobileWeHavePerfectService, MobileWhyPayLess, Dream CTA, FAQ.

         ── Phase 5 (Dec 2026) — REINSTATED via lightweight wrappers ──
         Per user requirement: EVERY block from the desktop welcome MUST
         appear on the mobile welcome (simplified, but present). The new
         wrappers live in `./MobileWelcomeBlocks` and ship mobile-tuned
         versions of:
           • Popular brands              (BrandLogos1)
           • Top vehicle deals           (VehicleDeals1 + FrameComponent21)
           • Quick calculator            (FrameComponent22)
           • Three ways to work          (FrameComponent23)
           • Turnkey / perfect service   (TurnkeyBanner1 + FrameComponent24)
           • Real deals / Before-After   (BeforeAfterSection)
           • Reviews                     (ReviewsArea1)
       */}

      {/* ═════════ POPULAR BRANDS ═════════ */}
      <MobilePopularBrands />

      {/* ═════════ TOP VEHICLE DEALS (weekly carousel) ═════════ */}
      <MobileTopDeals />

      {/* ═════════ QUICK CALCULATOR (all-in EUR) ═════════ */}
      <MobileQuickCalc />

      {/* ═════════ THREE WAYS WE WORK ═════════ */}
      <MobileThreeWays />

      {/* ═════════ TURNKEY DELIVERY (How we deliver) ═════════ */}
      <MobileTurnkeyProcess />

      {/* ═════════ PERFECT SERVICE — process steps (existing) ═════════ */}
      <MobileWeHavePerfectService />

      {/* ═════════ OUR SERVICES — full-cycle ═════════ */}
      <MobileOurServices />

      {/* ═════════ AVITO PROOF — Real deals, verified reviews ═════════ */}
      <MobileAvitoProof />
      ...
      {/* ═════════ FOOTER — shared MobileFooter component ═════════ */}
      <MobileFooter />
    </div>
  );
}
...
function MobileTurnkeyServices() {
  ...
  const CARDS = isRu
    ? [
        {
          num: '1',
          title: 'РЕАЛЬНЫЙ ОСМОТР И ПОДБОР',
          body:
            'Каждое авто проходит живой осмотр до отправки: ЛКП, геометрия кузова, скрытая коррозия, электроника, сверка VIN. Получаете именно то, что видели — без сюрпризов.',
        },
        {
          num: '2',
          title: 'ДОСТАВКА К ДОМУ',
          body:
            'Привозим автомобиль в любой город России или Беларуси. Ключи получаете прямо по своему адресу — без лишних поездок и таможенных очередей.',
        },
        {
          num: '3',
    ...
    {
      num: '1',
      title: 'REAL INSPECTION & HAND-PICK',
      body:
        'Every car goes through a live walk-around before shipping: paint thickness, frame geometry, hidden corrosion, electronics, VIN match. You receive exactly what you saw — no surprises.',
    },
    {
      num: '2',
      title: 'DOOR-TO-DOOR DELIVERY',
      body:
        'We deliver to any city in Russia or Belarus. Keys collected at your address — no extra trips, no border runs.',
    },
    {
      num: '3',
      title: 'CUSTOMS & STANDARDS',
      body:
        'Full customs clearance with documents matching EU export and RU / BY import standards. We handle every paper and every stamp.',
    },
    {
      num: '4',
      title: 'MINIMUM DEPOSIT',
      body:
        'Start with a small reservation fee instead of full pre-payment. The balance is settled on delivery — flexible terms for every budget.',
    },
    {
      num: '5',
      title: 'OPTIMAL ROUTE',
      body:
        'We build the most cost-efficient route from auction lot to your driveway — balancing price, transit time and customs windows.',
    },
    {
      num: '6',
      title: 'PARTS SOURCING',
      body:
        'Need a missing element, OEM replacement or upgrade? We source genuine, quality-grade parts straight from EU suppliers.',
    },
    {
      num: '7',
      title: 'FAST 24/7 RESPONSE',
      body:
        'Auto-quote in minutes, a manager on the line within the hour. Every request gets an answer the same day — weekends included.',
    },
  ];
      ...
      >
        <span
          ...
          style={{
            ...
            height: 32,
            borderRadius: 10,
            background: 'rgba(254, 174, 0, 0.16)',
            color: '#feae00',
            fontSize: 18,
            fontWeight: 800,
          }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
          ...
          >
            {googleReviewsCount ?? 31} {t?.googleReviews || 'verified reviews'}
          </span>
        </div>
      </div>
