 ...
 * DM Auto signature: bracket-framed tagline on the right, [N] corner numbering.
...
const PLANS = [
  {
    ...
    num: 1,
    tag: 'Standard',
    desc: 'Sourcing, inspection, bidding, purchase, and delivery to Belarus or Russia.',
...
export default function HowWeWorkBlock() {
  ...
  return (
    <section className="bg-[var(--bg-base)] py-24" data-testid="how-we-work-section">
      <div className="max-w-[1920px] mx-auto px-6 lg:px-[100px]">
        ...
        <div className="relative flex flex-col items-center mb-14">
          ...
          <div className="lg:absolute lg:right-0 lg:top-3 flex items-stretch gap-3 mt-6 lg:mt-0 max-w-[400px]">
            ...
            <p className="text-[12px] md:text-[13px] uppercase tracking-[0.06em] leading-snug">
              <span className="text-[var(--accent-brand)]">We work for each client</span>
              <br />
              <span className="text-[var(--text-primary)]">depending on the budget</span>
            </p>
            <span className="text-[var(--accent-brand)] text-[28px] leading-none font-light select-none">]</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {PLANS.map((p) => {
            ...
            const base = isAmber ? 'bg-[var(--accent-brand)]' : 'bg-[var(--bg-elevated)]';
            const bracketColor = isAmber ? 'text-black/70' : 'text-[var(--accent-brand)]';
            const titleColor = isAmber ? 'text-black' : 'text-[var(--accent-brand)]';
            const descColor = isAmber ? 'text-black/80' : 'text-[var(--text-secondary)]';
            const accentColor = isAmber ? 'text-black' : 'text-[var(--text-primary)]';
            return (
              <div
                key={p.key}
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => openGetInTouch({ source: `plan-${p.key}` })}
                  className={
                    isAmber
                      ? 'w-full h-[52px] inline-flex items-center justify-center gap-2 rounded bg-[var(--bg-base)] text-[var(--accent-brand)] font-semibold uppercase text-[13px] tracking-wider hover:bg-[var(--bg-elevated)] transition-colors'
                      : 'w-full h-[52px] inline-flex items-center justify-center gap-2 rounded bg-[var(--accent-brand)] text-black font-semibold uppercase text-[13px] tracking-wider hover:bg-[var(--accent-brand-hover)] transition-colors'
                  }
