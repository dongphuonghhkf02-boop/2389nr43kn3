...
// ── Fallback contacts — used only on first paint before /api/site-info returns
// or as a hard fallback if a particular field is empty in the CMS. The admin
// can override every single value via Admin → Information → Contacts.
const FALLBACK_CONTACTS = {
  whatsappPhone: '+375291234567',
  whatsappLabel: '+375 29 123 45 67',
  telegram: 'dm_auto_support',
  phone: '+375291234567',
...
// ── Каналы связи ───────────────────────────────────────────────────────
function buildChannels(t, contacts) {
  const c = contacts || FALLBACK_CONTACTS;
  const phoneClean = (c.whatsappPhone || FALLBACK_CONTACTS.whatsappPhone).replace(/\D/g, '');
  return [
    {
      key: 'whatsapp',
      Icon: WhatsappLogo,
      title: t.waTitle,
      hint: t.waHint,
      value: c.whatsappLabel || FALLBACK_CONTACTS.whatsappLabel,
      cta: t.waCta,
    ...
    },
    {
      ...
      title: t.tgTitle,
      hint: t.tgHint,
      value: `@${c.telegram || FALLBACK_CONTACTS.telegram}`,
      cta: t.tgCta,
      href: `https://t.me/${c.telegram || FALLBACK_CONTACTS.telegram}`,
      external: true,
      accent: 'blue',
    },
    {
      ...
      title: t.phoneTitle,
      hint: t.phoneHint,
      value: c.phoneLabel || FALLBACK_CONTACTS.phoneLabel,
      cta: t.phoneCta,
      href: `tel:${(c.phone || FALLBACK_CONTACTS.phone).replace(/\s+/g, '')}`,
      external: false,
      accent: 'navy',
    },
    {
      ...
      title: t.emailTitle,
      hint: t.emailHint,
      value: c.email || FALLBACK_CONTACTS.email,
      cta: t.emailCta,
      href: `mailto:${c.email || FALLBACK_CONTACTS.email}`,
      external: false,
      accent: 'amber',
    },
  ];
...
export default function ContactsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await axios.get(`${API_URL}/api/site-info`, { timeout: 12000 });
        if (!cancelled) setSiteInfo(r.data || null);
      } catch (_) { /* fallback to FALLBACK_CONTACTS — keep silent */ }
    })();
    return () => { cancelled = true; };
  }, []);

  // Resolve live contacts from /api/site-info → fallback to defaults.
  const fc = (siteInfo && siteInfo.footer && siteInfo.footer.contacts) || {};
  const phoneFromSite = (Array.isArray(fc.phones) && fc.phones[0]) || '';
  const phoneClean = phoneFromSite ? phoneFromSite.replace(/\s+/g, '') : '';
  const liveContacts = {
    whatsappPhone: fc.whatsapp_number || phoneClean || FALLBACK_CONTACTS.whatsappPhone,
    whatsappLabel: fc.whatsapp_label || fc.whatsapp_number || phoneFromSite || FALLBACK_CONTACTS.whatsappLabel,
    telegram: (fc.telegram_username || FALLBACK_CONTACTS.telegram).replace(/^@/, ''),
    phone: phoneClean || FALLBACK_CONTACTS.phone,
    phoneLabel: phoneFromSite || FALLBACK_CONTACTS.phoneLabel,
    email: fc.email || FALLBACK_CONTACTS.email,
  };

  // Working hours from CMS (RU/EN aware); falls back to copy default.
  const workingHours = (lang === 'ru'
    ? (fc.working_hours_ru || fc.working_hours)
    : (fc.working_hours || fc.working_hours_ru)) || t.hoursValue;

  const channels = buildChannels(t, liveContacts);
          ...
          <div className="bibi-c2-facts">
            <div className="bibi-c2-fact">
              ...
              <div>
                ...
                <div className="bibi-c2-fact__value" data-testid="contacts-hours-value">{workingHours}</div>
              </div>
            </div>
            <div className="bibi-c2-fact">
